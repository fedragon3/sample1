...
(function () {
  ...
  function chatSystemPrompt(rep) {
    return [
      ...
      "",
      "[언어 규정 - 필수 지침]",
      "1. 답변은 한국어(한글)로 작성합니다. 꼭 필요한 경우 영어 단어는 그대로 써도 됩니다.",
      "2. 중국어 한자(漢字), 일본어 문자(かな), 러시아어(키릴), 베트남어 등 한글·영어가 아닌 문자는 절대 쓰지 마세요.",
      "3. 전문 용어나 운세 용어도 괄호 한자 없이 한글로만 표기하세요(예: 사주, 오행, 바이오리듬).",
      "",
      "===== 사용자 운세 리포트(근거) =====",
      reportContext(rep),
      "===== 리포트 끝 =====",
  ...
  // 허용(allowlist) 문자만 남기는 정규식 — 한글 + 영어/숫자/기본 문장부호 + 공백 +
  // 자주 쓰는 기호·이모지. 이 집합에 없는 문자는 무엇이든(한자·가나·키릴·베트남어·
  // 아랍어 등) 한 번에 제거된다. 스크립트별로 일일이 막던 방식보다 견고하다.
  const KEEP_RE = /[^\uAC00-\uD7A3\u1100-\u11FF\u3130-\u318F\uA960-\uA97F\uD7B0-\uD7FF\u0020-\u007E\n\r\t\u2000-\u206F\u2190-\u21FF\u2300-\u23FF\u2460-\u24FF\u2600-\u27BF\u2B00-\u2BFF\uFE00-\uFE0F\uD800-\uDFFF]/g;

  function cleanText(str) {
    if (!str) return "";
    let clean = str;
    // 1. LLM 이 흘리는 외래어 단어를 자연스러운 한글로 치환(단순 삭제보다 의미 보존)
    //    예: показ합니다 -> 보여줍니다, nhìn보며 -> 바라보며, 바라하는 -> 바라보는
    FOREIGN_MAP.forEach((f) => { clean = clean.replace(f.pattern, f.replace); });
    // 2. 괄호 안이 한자/외국어뿐인 병기 제거: "운세(運勢)" -> "운세"
    clean = clean.replace(/\([\u4E00-\u9FFF\u3400-\u4DBF\uF900-\uFAFF\u3040-\u30FF\u0400-\u04FF\u00C0-\u024F\u1EA0-\u1EF9\s,.]+\)/g, "");
    // 3. 자주 쓰는 한자는 한글 음독으로 변환(삭제 대신 뜻 유지)
    clean = clean.replace(/[\u4E00-\u9FFF\u3400-\u4DBF\uF900-\uFAFF]/g, (ch) => HANJA_TO_HANGUL[ch] || "");
    // 4. 허용 집합에 없는 모든 문자 제거(한글·영어 외 스크립트 일괄 차단)
    clean = clean.replace(KEEP_RE, "");
    // 5. 제거로 생긴 군더더기 공백 정리
  ...
  function tSystemPrompt() {
    ...
    return [
      ...
      "1. 답변은 한국어(한글)로 작성합니다. 꼭 필요한 경우 영어 단어는 그대로 써도 됩니다.",
      "2. 중국어 한자(漢字), 일본어 문자(かな), 러시아어(키릴), 베트남어 등 한글·영어가 아닌 문자는 절대 쓰지 마세요.",
      "3. '바라보다', '들여다보다', '관찰하다' 처럼 매끄러운 한글 어휘를 사용하세요.",
    ].join("\n");
  }
